---
title: "Analysis Revision - Sensitivity"
...
Four participants were removed due to writing a four-letter word that started with g other than "grid" in the attention check.
...
# (Intercept)     3.129925   0.237533   13.18   <2e-16 ***
# Frame_Quantity -0.026880   0.002042  -13.16   <2e-16 ***

#OLD
#                 Estimate Std. Error z value Pr(>|z|)    
# (Intercept)     2.978545   0.216501   13.76   <2e-16 ***
# Frame_Quantity -0.025610   0.001765  -14.51   <2e-16 ***
```
...
Five participants were removed due to writing a four-letter word that started with g other than "grid" in the attention check.

```{r MEANRANGE ACC}
dfaccID2 <- as_tibble(Exp1_ID2_df %>% 
   dplyr::filter(ResponseId %ni% c("R_1r9BuWVyRnc3csX",
                                  "R_27NFR4bp1E924Uh",
                                  "R_1CI5Hw5N4mNzxqU",
                                  "R_2ZWAu0i7xJwq1M5",
                                  "R_BLp0vDkvzXeo81z")) %>% 
...
Three participants were removed due to writing a four-letter word that started with g other than "grid" in the attention check.
...
One participant was removed due to writing a four-letter word that started with g other than "grid" in the attention check.
...
Seven participants were removed due to either writing a four-letter word that started with g other than "grid" in the attention check or responding with values outside of the maximum power value possible.
...
dfaccS2 <- as_tibble(Exp1_S2_df %>% 
  dplyr::filter(ResponseId %ni% c("R_3q87De0UcHnqWcD",
                                  ...
                                  "R_1JJf0yMHhxY1vPi",
                                  "R_2X0t3WmKsvdeDuO",
                                  "R_3m9uN5B0pCg6A84", #Had  trials outside of MaxRange
                                  "R_erqfEjsMzi4ChQ"  #Had trials outside of MaxRange
                                  )) %>%
...
df_fullId1 <- df_full %>%
  dplyr::mutate(task = factor(task, levels = c("ID1","ID2","ID3","C1","C2","S1")))
modfull_Id1 <- glmer(acc ~ task * Frame_Quantity + (1|Trial) + (1|ResponseId), data = df_fullId1,
...
# ID2 referent -------------
 df_fullId2 <- df_full %>%
   dplyr::mutate(task = fct_relevel(task, "ID2", "ID1","ID3","C1","C2","S1"))
 modfull_Id2 <- glmer(acc ~ task * Frame_Quantity + (1|Trial) + (1|ResponseId), data = df_fullId2,
                  family = binomial(link = "logit"))
summary(modfull_Id2)
```
...
dfacc <- as_tibble(Exp2_S2_df %>% 
  dplyr::mutate(error = abs(as.numeric(Response,na.rm = T) - as.numeric(Correct_Mean,na.rm = T))) %>%
    dplyr::mutate(error = 1 - error/MaxRange) %>% 
    dplyr::filter(error < 3*sd(error)+ mean(error), error > 0) %>% 
   dplyr::mutate(across(c("Trial", "Frame_Quantity","time_lim"),.fns = as.numeric),
...
summ2_plot <- ggplot(dfsum, aes(x=Frame_Quantity, y=error, fill = time_lim))+ #setting up x and y axes
    ...
    ylab("Proportional Accuracy")+ #naming y-axis
    xlab("Number of Frames")+ #naming x-axis
    ggtitle("Experiment 1 (Summary 2)")+  #naming the figure (or the plot)
    theme_tidybayes() + #theme of the plot
    facet_grid(scale~.) +
...
summ2_plot_scale <- ggplot(dfsum_scale, aes(x=scale, y=error, fill=scale),  show.legend = FALSE)+ #setting up x and y axes
    ...
    ylab("Proportional Accuracy (0-1)")+ #naming y-axis
    xlab("Scale")+ #naming x-axis
